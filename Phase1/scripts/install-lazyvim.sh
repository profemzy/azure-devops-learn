#!/bin/bash
# Phase 1: Install LazyVim on Ubuntu/Debian
...
install_package "ripgrep" || apt-get install -y ripgrep >/dev/null 2>&1
install_package "fd-find" || apt-get install -y fd-find >/dev/null 2>&1
...
# Rust/Cargo (for Rust LSP and tools)
# Go (for Go LSP)
...
if command -v nvim &> /dev/null; then
    ...
    if [ "$NVIM_VERSION" \< "0.11.2" ]; then
        ...
        log_info "Will install latest Neovim..."
        if [ "$AS_ROOT" = true ]; then
...
if [ "$NVIM_NEEDS_UPDATE" = true ]; then
    ...
    # Try to install from Ubuntu repository first
    log_info "Installing Neovim from Ubuntu repository..."
    if [ "$AS_ROOT" = true ]; then
        apt-get update >/dev/null 2>&1 || true
        apt-get install -y neovim >/dev/null 2>&1 || true
    else
        sudo apt-get update >/dev/null 2>&1 || true
        sudo apt-get install -y neovim >/dev/null 2>&1 || true
    fi

    # Check if repository version is new enough
    if command -v nvim &> /dev/null; then
        INSTALLED_VERSION=$(nvim --version | head -n1 | awk '{print $2}')
        if [ "$INSTALLED_VERSION" \>= "0.11.2" ]; then
            log_success "Neovim $INSTALLED_VERSION installed from repository"
        else
            log_warning "Repository version is too old ($INSTALLED_VERSION), using AppImage..."
            if [ "$AS_ROOT" = true ]; then
                rm -f /usr/local/bin/nvim /usr/bin/nvim
            else
                sudo rm -f /usr/local/bin/nvim /usr/bin/nvim
    ...
    # If we still need Neovim, use tarball
    if ! command -v nvim &> /dev/null; then
        # Fallback: Download Neovim tarball
        log_info "Downloading Neovim tarball..."
        cd /tmp
...
if ! command -v lazygit &> /dev/null; then
    ...
    if [ -z "$LAZYGIT_URL" ]; then
        ...
        if curl -fL -o lazygit.tar.gz "$LAZYGIT_URL" 2>&1; then
            ...
            if [ -f "lazygit" ]; then
                ...
                log_warning "Failed to extract lazygit binary"
            fi
            rm -f lazygit.tar.gz
        else
...
echo "=========================================="
echo ""
log_success "LazyVim and build tools have been installed!"
echo ""
